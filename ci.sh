...
#if [[ -z ${CI_OPT_MAVEN_BUILD_REPO+x} ]]; then CI_OPT_MAVEN_BUILD_REPO="${CI_OPT_GIT_PREFIX}/ci-and-cd/maven-build/raw/v2.0.0"; fi
if [[ -z ${CI_OPT_MAVEN_EFFECTIVE_POM+x} ]]; then CI_OPT_MAVEN_EFFECTIVE_POM="false"; fi
if [[ -z ${CI_OPT_ORIGIN_REPO_SLUG+x} ]]; then if [[ -n "${CI_PROJECT_PATH}" ]]; then CI_OPT_ORIGIN_REPO_SLUG="gitlab-cloud-ready/spring-boot-starter-rate-limiter"; else CI_OPT_ORIGIN_REPO_SLUG="cloud-ready/spring-boot-starter-rate-limiter"; fi; fi
if [[ -z ${CI_OPT_SITE+x} ]]; then CI_OPT_SITE="true"; fi
if [[ -z "${CI_OPT_SITE_PATH_PREFIX}" ]] && [[ "${CI_OPT_GITHUB_SITE_PUBLISH}" == "true" ]]; then
    # github site repo cloud-ready/cloud-ready
