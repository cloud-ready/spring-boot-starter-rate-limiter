...
if [ -z "${CI_OPT_ORIGIN_REPO_SLUG}" ]; then CI_OPT_ORIGIN_REPO_SLUG="cloud-ready/spring-boot-starter-rate-limiter"; fi
...
if [ -z "${CI_OPT_SITE_PATH_PREFIX}" ] && [ "${CI_OPT_GITHUB_SITE_PUBLISH}" == "true" ]; then
    ...
    CI_OPT_SITE_PATH_PREFIX="cloud-ready/spring-boot-starter-rate-limiter"
fi
